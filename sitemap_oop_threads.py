...
def list_urls_from_bd(id_domain):
    db = MySQLi(host, user, password, database_home)
    all_urls_for_domain_in_db = db.fetch("SELECT url FROM sitemap_all WHERE id = %s", id_domain)
    urls_in_bd = []
...
def finally_threads_sitemap(site):
    ...
    Если переменная id_domain_from_date будет пустой, то парси
    нга не было.
    """
    ...
    if len(id_domain_from_date['rows']) < 1:
        ...
        urls_in_db = list_urls_from_bd(id_domain)
        ...
        for url in sitemaps_urls['urls_list']:
            if url in urls_in_db:
                ...
                num_add_urls += 1
                db.commit("INSERT INTO sitemap_all (id, url) VALUES (%s, %s)", id_domain, url)
        ...
        if num_add_urls >= 1 and num_add_urls != count_sitemaps:
            for url_bd in sitemaps_urls['urls_list']:
                if url_bd not in urls_in_db:
                    num_add_urls += 1
                    db.commit("INSERT INTO sitemap_add (id, url, date) VALUES (%s, %s, %s)", id_domain, url_bd,
                              date_for_bd)
        ...
        for url in urls_in_db:
            if url not in sitemaps_urls['urls_list']:
                print("Эту ссылку надо удалить. Она пропала в карте сайта.")
                print(url)
                db.commit("DELETE FROM sitemap_all WHERE url = %s", url)
                db.commit("INSERT INTO sitemap_del(id, url, date) VALUES(%s, %s, %s)", id_domain, url, date_for_bd)
